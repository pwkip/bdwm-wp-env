...
/**
 * bdwm-wp-env configuration.
 *
 * @typedef WPConfig
 * @property {string}                           name                    Name of the environment.
 * @property {string}                           configDirectoryPath     Path to the .bdwm-wp-env.json file.
 * @property {string}                           workDirectoryPath       Path to the work directory located in ~/.bdwm-wp-env.
 * @property {string}                           dockerComposeConfigPath Path to the docker-compose.yml file.
 * @property {boolean}                          detectedLocalConfig     If true, bdwm-wp-env detected local config and used it.
...
/**
 * Reads, parses, and validates the given .bdwm-wp-env.json file into a bdwm-wp-env config
 * object for internal use.
 *
 * @param {string} configPath Path to the .bdwm-wp-env.json file.
 *
 * @return {WPConfig} A parsed and validated bdwm-wp-env config object.
 */
module.exports = async function readConfig( configPath ) {
	...
	// Default configuration which is overridden by .bdwm-wp-env.json files.
	...
	// The specified base configuration from .bdwm-wp-env.json or from the local
	// source type which was automatically detected.
	const baseConfig =
		( await readRawConfigFile( '.bdwm-wp-env.json', configPath ) ) ||
		( await getDefaultBaseConfig( configPath ) );

	// Overriden .bdwm-wp-env.json on a per-user case.
	const overrideConfig =
		( await readRawConfigFile(
			'.bdwm-wp-env.override.json',
			configPath.replace( /\.bdwm-wp-env\.json$/, '.bdwm-wp-env.override.json' )
		) ) || {};
	...
	if ( allPorts.size !== allEnvs.length ) {
		throw new ValidationError(
			'Invalid .bdwm-wp-env.json: Each port value must be unique.'
		);
	}
	...
	const mergeNestedObjs = ( key ) =>
		Object.assign(
			...
			...configs.map( ( config ) => {
				...
				throw new ValidationError(
					`Invalid .bdwm-wp-env.json: "${ key }" must be an object.`
				);
			} )
		);
...
/**
 * Detects basic config options to use if the .bdwm-wp-env.json config file does not
	...
	const updateEnvUrl = ( configKey ) => {
		[ 'development', 'tests' ].forEach( ( envKey ) => {
			try {
				...
				throw new ValidationError(
					`Invalid .bdwm-wp-env.json: config.${ configKey } must be a valid URL.`
				);
			}
		} );
	};
...
/**
 * Gets the `bdwm-wp-env` home directory in which generated files are created.
 *
 * By default: '~/.bdwm-wp-env/'. On Linux with snap packages: '~/bdwm-wp-env/'. Can be
 * overriden with the WP_ENV_HOME environment variable.
 *
 * @return {Promise<string>} The absolute path to the `bdwm-wp-env` home directory.
 */
