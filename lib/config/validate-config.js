 ...
 * while reading bdwm-wp-env configuration.
 */
...
function validateConfig( config, envLocation ) {
	...
	if ( config.core !== null && typeof config.core !== 'string' ) {
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }core" must be null or a string.`
		);
	}
	...
	if (
		...
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }plugins" must be an array of strings.`
		);
	}
	...
	if (
		...
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }themes" must be an array of strings.`
		);
	}

	if ( ! Number.isInteger( config.port ) ) {
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }port" must be an integer.`
		);
	}

	if ( typeof config.config !== 'object' ) {
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }config" must be an object.`
		);
	}

	if ( typeof config.mappings !== 'object' ) {
		throw new ValidationError(
			`Invalid .bdwm-wp-env.json: "${ envPrefix }mappings" must be an object.`
		);
	}
	...
	for ( const [ wpDir, localDir ] of Object.entries( config.mappings ) ) {
		if ( ! localDir || typeof localDir !== 'string' ) {
			throw new ValidationError(
				`Invalid .bdwm-wp-env.json: "${ envPrefix }mappings.${ wpDir }" should be a string.`
			);
		}
	}
	return config;
