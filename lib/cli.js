...
module.exports = function cli() {
	...
	yargs.command(
		...
		wpGreen(
			chalk`Starts WordPress for development on port {bold.underline ${ terminalLink(
				...
				'8889',
				'http://localhost:8889'
			) }} (override with WP_ENV_TESTS_PORT). The current working directory must be a WordPress installation, a plugin, a theme, or contain a .bdwm-wp-env.json file. After first insall, use the '--update' flag to download updates to mapped sources and to re-apply WordPress configuration options.`
		),
		( args ) => {
			args.option( 'update', {
				type: 'boolean',
	...
	yargs.command(
		...
		'Runs an arbitrary command in one of the underlying Docker containers. For example, it can be useful for running wp cli commands. You can also use it to open shell sessions like bash and the WordPress shell in the WordPress instance. For example, `bdwm-wp-env run cli bash` will open bash in the development WordPress instance.',
		( args ) => {
